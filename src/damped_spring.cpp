...
double DampedSpring::update(const double p_delta, const double p_value, const double p_target_value, const Ref<DampedSpringParameters> &p_parameters) {
	...
	const double acceleration = -p_parameters->get_spring_constant() * (p_value - p_target_value) - p_parameters->get_damping_constant() * velocity;
	...
	double new_value = p_value + velocity * p_delta;
