...
- `src/damped_spring_math.h` — header-only, engine-independent template function `damped_spring_update<T>(delta, value, target_value, spring_constant, damping_constant, r_velocity)` implementing the one semi-implicit Euler step (`F = -k*(value - target) - c*velocity`, mass = 1) shared by all three spring classes: it advances `r_velocity` in place by `acceleration * delta` and returns `value + r_velocity * delta`. Templated on `T` (only `+`, `-`, and scalar `*` are used) so it works for `double`/`Vector2`/`Vector3` alike, and takes the already-derived `spring_constant`/`damping_constant` coefficients rather than a `DampedSpringParameters` reference, so it has no godot-cpp dependency — this is what lets `tests/test_damped_spring.cpp` include and exercise it as a plain native program. Also has `damped_spring_update_blend<T>(delta, value, target_value, spring_constant_a, damping_constant_a, spring_constant_b, damping_constant_b, weight, r_velocity)`, which linearly interpolates each constant between the `_a`/`_b` pair by `weight` (its own inline lerp, not `Math::lerp`, to keep the header dependency-free) and calls `damped_spring_update()` with the blended pair — for crossfading between two tunings over time instead of snapping between them.
- `src/damped_spring_parameters.h`/`.cpp` — `DampedSpringParameters : public Resource`, a standalone, shareable config object (`GDREGISTER_CLASS`, instantiable directly from GDScript, e.g. `DampedSpringParameters.new()`). Owns the `time`/`damping` properties: `time` is the characteristic resolve time (seconds; clamped to a small positive minimum), and `damping` is the deviation from critically damped (`0.0` = critically damped, `> 0.0` = overdamped, `< 0.0` = underdamped/oscillatory, clamped so the effective damping ratio never goes negative). `setup(time, damping)` sets both and recomputes the private `spring_constant`/`damping_constant` fields (`k = 1/time²`, `c = 2·(1+damping)/time`, mass = 1) in one pass; `set_time`/`set_damping` each call `setup()` under the hood. `get_spring_constant()`/`get_damping_constant()` expose the cached coefficients for other classes to read (in particular, to pass into `damped_spring_update()`) and are also bound as read-only `spring_constant`/`damping_constant` properties (`ADD_PROPERTY` with an empty setter string and `PROPERTY_USAGE_EDITOR | PROPERTY_USAGE_READ_ONLY`, so they show in the inspector but aren't settable or serialized, since they're always re-derived from `time`/`damping`). Being a plain `Resource` (not tied to any one spring instance) means one `DampedSpringParameters` can be shared across many springs — e.g. all UI elements using the same "snappy" tuning reference the same resource.
- `src/damped_spring.h`/`.cpp`, `src/damped_spring_2d.h`/`.cpp`, `src/damped_spring_3d.h`/`.cpp` — `DampedSpring`/`DampedSpring2D`/`DampedSpring3D : public Object`, damped harmonic oscillators for `double`/`Vector2`/`Vector3`. Each holds *only* `velocity` (typed to match the class) as a bound property — no value, no target, and no spring/damping tuning of its own, so the same instance can be reused for a different value/target/parameters each call (`velocity` is the one piece of state that must persist between calls; everything else is a per-call input). `update(delta, value, target_value, parameters)` (bound as `update(p_delta, p_value, p_target_value, p_parameters)`) is a thin wrapper: after an `ERR_FAIL_COND_V` guard on a null `parameters` ref (returning `p_value` unchanged), it calls `damped_spring_update(p_delta, p_value, p_target_value, p_parameters->get_spring_constant(), p_parameters->get_damping_constant(), velocity)` from `damped_spring_math.h` — the caller owns storing the returned value (e.g. into `sprite.position`) and passing it back in on the next call. `update_blend(delta, value, target_value, parameters_a, parameters_b, weight)` is the same shape but guards both `parameters_a`/`parameters_b` against null and calls `damped_spring_update_blend()` instead, for crossfading smoothly between two `DampedSpringParameters` tunings by `weight` (`0.0` = fully `parameters_a`, `1.0` = fully `parameters_b`) rather than switching between them outright. `update_constants(delta, value, target_value, spring_constant, damping_constant)` skips `DampedSpringParameters` entirely and calls `damped_spring_update()` with caller-supplied coefficients directly — no null to guard against, since there's no `Ref` involved; meant for a caller that already has (or has computed/blended) its own `spring_constant`/`damping_constant` pair, e.g. read from `DampedSpringParameters.spring_constant`/`.damping_constant`. Because they derive from `Object` rather than `Node`/`Resource`, they're meant to be instantiated (e.g. `DampedSpring.new()`) and driven manually — not placed in the scene tree or saved as a resource file. These are also the reference pattern for any new class in this codebase: `GDCLASS(ClassName, BaseClass)` macro, a `_bind_methods()` static method registering properties/methods via `ClassDB::bind_method` + `ADD_PROPERTY`, plain getter/setter pairs per property.
- `tests/test_damped_spring.cpp` — a native (non-Godot) convergence test that calls `damped_spring_update()` from `src/damped_spring_math.h` directly against a plain `double` (mirroring the scalar `DampedSpring` case): simulates a spring tuned to settle in ~1 second using several fixed delta times (1/24s down to 1/4800s) and checks each result lands close to the closed-form critically-damped solution, and that error shrinks as delta shrinks — i.e. `update()`'s behavior doesn't depend on a specific fixed timestep. Built via a separate, plain (non-cross-compiling) `Environment()` in `SConstruct` — the `tests` SCons target is independent of the GDExtension's `env`/`platform`/`target` build variables since the test binary needs to run on the host machine, not the GDExtension's target platform.
